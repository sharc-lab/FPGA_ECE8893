    ...
    INPUT_BUFFER_WIDTH:
        ...
        INPUT_BUFFER_DEPTH:
            for (int c = 0; c < IN_BUF_DEPTH; c++)
            {
                ...
                if (((ti == 0) && (i < 1)) || ((tj == 0) && (j < 1)) || ((ti == N_TILE_ROWS - 1) && (i > IN_BUF_HEIGHT - P)) || ((tj == N_TILE_COLS - 1) && (j > IN_BUF_WIDTH - P)))
                    in_fm_buf[c][i][j] = 0;
                ...
                // if (ti == 0 && tj == 0)
                //     std::cout << "In buff value: " << in_fm_buf[c][i][j] << "\n";
            }
        }
    }
}
    ...
    OUTPUT_BUFFER_WIDTH:
        ...
        OUTPUT_BUFFER_DEPTH:
            for (int f = 0; f < OUT_BUF_DEPTH; f++)
            {
                ...
                {
                    // if (ti == 0 && tj == 0)
                    //     std::cout << "Out buff value: " << out_fm_buf[f][i][j] << "\n";
                    if (out_fm_buf[f][i][j] >= (fm_t)0)
