...
float conv_layer_weights_l1[32][3][3][3];
float conv_layer_bias_l1[32];
float conv_layer_weights_l2[64][32][3][3];
float conv_layer_bias_l2[64];
float conv_layer_golden_output_feature_map[64][104][104];

fm_t  fixp_conv_layer_input[3][416][416];
fm_t  fixp_conv_layer_input_feature_map[1024][416][416];
wt_t  fixp_conv_layer_weights_l1[32][1024][3][3];
wt_t  fixp_conv_layer_bias_l1[1024];

wt_t  fixp_conv_layer_weights_l2[64][1024][3][3];
wt_t  fixp_conv_layer_bias_l2[1024];

fm_t  fixp_conv_layer_output_feature_map[1024][416][416] = {0};
fm_t  fixp_conv_layer_output_L12_feature_map[64][104][104] = {0};
...
void read_bin_files()
{
    ...
    ifs_conv_weights1.read((char*)(***conv_layer_weights_l1), 32*3*3*3*sizeof(float));
    ...
    ifs_conv_bias1.read((char*)(conv_layer_bias_l1), 32*sizeof(float));
    ...
    ifs_conv_weights2.read((char*)(***conv_layer_weights_l2), 32*64*3*3*sizeof(float));
    ...
    ifs_conv_bias2.read((char*)(conv_layer_bias_l2), 64*sizeof(float));
    ...
    // Golden Output
    ifstream ifs_golden_output("./conv_layer2_output.bin", ios::in | ios::binary);
    ifs_golden_output.read((char*)(**conv_layer_golden_output_feature_map), 64*104*104*sizeof(float));    
...
int main ()
{
    ...
    yolov4_tiny (fixp_conv_layer_input,
		...
		fixp_conv_layer_output_L12_feature_map,
        ...
        for(int i = 0; i < 104; i++)
        {
            for(int j = 0; j < 104; j++)
            {
                mse += std::pow((conv_layer_golden_output_feature_map[f][i][j] 
                                 -(float) fixp_conv_layer_output_L12_feature_map[f][i][j]), 2);
