...
fm_t fixp_conv_layer_output_L5_concat[2 * OUT_FM_DEPTH][OUT_FM_HEIGHT][OUT_FM_WIDTH] = {0};
//--------------------------------------------------------------------------
...
{
    // Input Feature Map
    ifstream ifs_conv_input("./bin/layer4_output.bin", ios::in | ios::binary);
    // ifstream ifs_conv_input("./bin/conv_layer5_input.bin", ios::in | ios::binary);
    ...
    // Input Feature Map
    ifstream ifs_conv_input2("./bin/layer4_output.bin", ios::in | ios::binary);
    // ifstream ifs_conv_input("./bin/conv_layer5_input.bin", ios::in | ios::binary);
    ifs_conv_input2.read((char *)(**fixp_conv_layer_input_feature_map_outL4), IN_FM_DEPTH * IN_FM_HEIGHT * IN_FM_WIDTH * sizeof(float));
    ...
    // Weights
    ifstream ifs_conv_weights("./bin/fused_conv5_bn5_weights2.bin", ios::in | ios::binary);
    ...
    // Bias
    ifstream ifs_conv_bias("./bin/fused_conv5_bn5_bias2.bin", ios::in | ios::binary);
    ...
    // Golden Output
    ifstream ifs_golden_output("./bin/conv_layer5_output.bin", ios::in | ios::binary);
...
int main()
{
    ...
    std::cout << "----------------------------------------" << std::endl;
#ifdef CSIM_DEBUG
    if (mse > 0 && mse < std::pow(10, -10))
    {
        std::cout << "Floating-point Simulation Layer5 SUCCESSFUL!!!" << std::endl;
    }
    else
    {
        std::cout << "Floating-point Simulation Layer5 FAILED :(" << std::endl;
    }
#else
    if (mse > 0 && mse < std::pow(10, -3))
    {
        std::cout << "Fixed-point Simulation Layer5 SUCCESSFUL!!!" << std::endl;
    }
    else
    {
        std::cout << "Fixed-point Simulation Layer5 FAILED :(" << std::endl;
    }
#endif
    std::cout << "----------------------------------------" << std::endl;
    /*
        for (int f = 0; f < OUT_FM_DEPTH; f++)
        {
            ...
            */
    // CONCAT LAYER5 OUTPUT with LAYER5 input
    for (int f = 0; f < 2 * OUT_FM_DEPTH; f++)
        ...
        for (int i = 0; i < OUT_FM_HEIGHT; i++)
            ...
            {
                if (f < 32)
                    fixp_conv_layer_output_L5_concat[f][i][j] = fixp_conv_layer_output_feature_map[f][i][j];
                else
                    fixp_conv_layer_output_L5_concat[f][i][j] = fixp_conv_layer_input_feature_map[f - 32][i][j];
            }

    // Convert to bin file
    // open the binary file for writing
    ofstream outfile1("./bin/layer5_output_concat.bin", ios::binary);
    outfile1.write((char *)(**fixp_conv_layer_output_L5_concat), (2 * OUT_FM_DEPTH) * OUT_FM_HEIGHT * OUT_FM_WIDTH * sizeof(float));
    outfile1.close();
    return 0;
}
