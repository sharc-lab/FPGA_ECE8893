...
float conv_layer_input_feature_map_split[IN_FM_DEPTH][IN_FM_HEIGHT][IN_FM_WIDTH];
//--------------------------------------------------------------------------
...
{
    // Input Feature Map
    ifstream ifs_conv_input2("./bin/layer3_output_split.bin", ios::in | ios::binary);
    ifs_conv_input2.read((char *)(**conv_layer_input_feature_map_split), IN_FM_DEPTH * IN_FM_HEIGHT * IN_FM_WIDTH * sizeof(float));
    ifs_conv_input2.close();
    /////////////////////
    // Input Feature Map
    ifstream ifs_conv_input("./bin/conv_layer4_input2.bin", ios::in | ios::binary);
    // ifstream ifs_conv_input("./bin/layer3_output_split.bin", ios::in | ios::binary);
    ...
    // Weights
    ifstream ifs_conv_weights("./bin/fused_conv4_bn4_weights2.bin", ios::in | ios::binary);
    ...
    // Bias
    ifstream ifs_conv_bias("./bin/fused_conv4_bn4_bias2.bin", ios::in | ios::binary);
    ...
    // Golden Output
    ifstream ifs_golden_output("./bin/conv_layer4_output2.bin", ios::in | ios::binary);
...
int main()
{
    ...
    {
        std::cout << "Floating-point Simulation Layer4 SUCCESSFUL!!!" << std::endl;
    }
    else
    {
        std::cout << "Floating-point Simulation Layer4 FAILED :(" << std::endl;
    }
#else
    if (mse > 0 && mse < std::pow(10, -3))
    {
        std::cout << "Fixed-point Simulation Layer4 SUCCESSFUL!!!" << std::endl;
    }
    else
    {
        std::cout << "Fixed-point Simulation Layer4 FAILED :(" << std::endl;
    }
#endif
    std::cout << "----------------------------------------" << std::endl;

    // open the binary file for writing
    ofstream outfile1("./bin/layer4_output.bin", ios::binary);
