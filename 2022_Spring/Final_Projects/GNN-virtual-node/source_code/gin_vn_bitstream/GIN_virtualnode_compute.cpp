...
#pragma HLS inline off
    ...
    FM_TYPE sum[VN_MLP_2_IN];
    FM_TYPE psum[VN_MLP_1_IN];

#pragma HLS array_partition variable=psum complete
#pragma HLS array_partition variable=vn_embedding2 complete //dim=1
#pragma HLS array_partition variable=vn_embedding1 complete //dim=1
...
#pragma HLS pipeline
        ...
        sum[dim] = virtualnode_mlp_1_bias[layer][dim];
    }
...
#pragma HLS pipeline
        for(int dim_in1 = 0; dim_in1 < VN_MLP_1_IN; dim_in1++) {
            ...
            sum[dim] += psum[dim_in1];
        }
        sum[dim] = sum[dim] < 0 ? (FM_TYPE)0 : sum[dim];
    }
...
#pragma HLS pipeline
        ...
        for(int dim_out2 = 0; dim_out2 < VN_MLP_2_OUT; dim_out2++) {
            vn_embedding1[dim_out2] += sum[dim] * virtualnode_mlp_2_weights[layer][dim_out2][dim];
        }
    }
...
#pragma HLS INTERFACE m_axi depth=50 port=gnn_node_mlp_1_weights_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=10 port=gnn_node_mlp_1_bias_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=50 port=gnn_node_mlp_2_weights_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=5 port=gnn_node_mlp_2_bias_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=865 port=gnn_node_embedding_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=325 port=gnn_edge_embedding_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=5 port=graph_pred_linear_weight_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=1 port=graph_pred_linear_bias_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=1 port=eps_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=50 port=gnn_node_virtualnode_mlp_1_weights_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=10 port=gnn_node_virtualnode_mlp_1_bias_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=50 port=gnn_node_virtualnode_mlp_2_weights_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=5 port=gnn_node_virtualnode_mlp_2_bias_fixed offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=5 port=gnn_node_virtualnode_embedding_weight_fixed offset=slave bundle=mem
...
#pragma HLS bind_storage variable=message1 type=RAM_2P impl=bram
#pragma HLS bind_storage variable=message2 type=RAM_2P impl=bram
...
// #pragma HLS bind_storage variable=vn_embedding2 type=RAM_2P impl=bram
    ...
    // perform VN_MLP and convolution once 
    virtualnode_MLP(vn_embedding1, vn_embedding2, 0);
    compute_CONV_layer(vn_embedding1, vn_embedding2, num_of_nodes, num_of_edges, 0);
